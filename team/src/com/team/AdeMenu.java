...
public class AdeMenu extends JPanel{
	...
	public AdeMenu(DBConn conn,OrderMenu orderMenu) {
    ...
    private void addMenu() {
        String sql = "SELECT * FROM MENU WHERE MENU_NUM > 3000 AND MENU_NUM < 4000";
        try {
    ...
    private void addMenuButton(String text, int index, int menuNum, String menuName, int menuPrice) {
    	ImageIcon icon = null;
    	if(menuName.equals("레몬에이드")) {
            icon = new ImageIcon("./image/remon_ade.png"); // 이미지 파일 경로를 설정하세요.
        } else if (menuName.equals("자몽에이드")) {
            icon = new ImageIcon("./image/grapefruit_ade.png"); // 다른 메뉴에 대한 기본 이미지 설정
        }  else if (menuName.equals("청포도에이드")) {
            icon = new ImageIcon("./image/grape_ade.png"); // 기본 이미지 설정
        } else if (menuName.equals("오렌지에이드")) {
            icon = new ImageIcon("./image/orange_ade.png"); // 기본 이미지 설정
        } else if (menuName.equals("블루베리에이드")) {
            icon = new ImageIcon("./image/blueberry_ade.png"); // 기본 이미지 설정
        } else  {
